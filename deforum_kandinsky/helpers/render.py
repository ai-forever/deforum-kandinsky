...
def render_animation(root, anim_args, args, cond_prompts, uncond_prompts):
    ...
    args.prior_seed = random.randint(0, int(1e6)) if args.prior_seed == -1 else args.prior_seed
    ...
    while frame_idx < anim_args.max_frames:
        current_params = dict(
            ...
            translation_z = keys.translation_z_series[frame_idx], 
            rotation_x = keys.rotation_3d_x_series[frame_idx], 
            rotation_y = keys.rotation_3d_y_series[frame_idx], 
            rotation_z = keys.rotation_3d_z_series[frame_idx], 
        ...
        if turbo_steps > 1:
            ...
            for tween_frame_idx in range(tween_frame_start_idx, frame_idx):
                ...
                output_img = cv2.cvtColor(img.astype(np.uint8), cv2.COLOR_RGB2BGR)
